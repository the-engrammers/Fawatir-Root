...
import { fournisseursList } from "@/lib/mock-data";
          ...
          <tbody className="divide-y divide-ink-200/60">
            {fournisseursList.map((f) => (
              <tr key={f.id} className="group">
                <td className="py-3 font-medium text-ink-900">{f.nom}</td>
                <td className="py-3 text-ink-700">{f.entreprise}</td>
                <td className="py-3 text-ink-500">{f.email}</td>
                <td className="figure py-3 text-ink-500">{f.telephone}</td>
