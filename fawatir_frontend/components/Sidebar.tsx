...
export default function Sidebar() {
  ...
  return (
    <aside className="w-[220px] h-full flex flex-col relative z-20 overflow-hidden">
      {/* Brand */}
      <div className="mb-6 px-3 mt-4 shrink-0">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide px-2 pb-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center gap-2.5 rounded-xl px-3 py-2 text-[12.5px] font-medium transition-all duration-300 ${
                  isActive
                    ? "bg-white/60 text-ink-900 shadow-sm backdrop-blur-md font-semibold"
                    : "text-ink-500 hover:bg-white/40 hover:text-ink-900"
                }`}
              >
                <item.icon size={15} className={isActive ? "text-brass" : "text-ink-400"} />
                {item.label}
              </Link>
            );
          })}
        ...
        {/* Bottom Navigation */}
        <div className="mt-6 mb-2 border-t border-ink-100/50 pt-4">
          <nav className="space-y-0.5">
      ...
      {/* User Profile */}
      <Link href="/entreprise" className="shrink-0 group flex flex-col items-start rounded-[24px] bg-white/40 p-3 m-2 mt-0 hover:bg-white/60 hover:shadow-spatial active:scale-95 transition-all duration-500 backdrop-blur-xl border border-white/50 relative overflow-hidden">
